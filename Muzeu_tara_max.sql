...
create or replace package body best_furnizor is
  ...
  begin
    ...
    open p_cursor for 
      select m.nume,l.tara from detinator m
        join artefact a on a.ID_DETINATOR=m.ID 
        join LOCATIE l on l.ID=a.ID_LOCATIE
        where m.tip='Muzeu' 
        group by m.id,m.nume,l.tara
...
begin
  ...
  loop
    ...
    dbms_output.put_line(l_nume||' '||l_tare);
  end loop;
  close l_cursor;
end;*/